...
6. **packer** — μαζεύει τα πάνω σε ένα γενναιόδωρο `pack.json` (περιλαμβάνει top errors
   **με τα μηνύματά τους**, week-over-week deltas) → S3 PutObject.
...
- **Steps:** read latest `pack.json` → investigate (συσχετίσεις/εξήγηση/priority,
  ελληνικό narrative + AI insights section) → render self-contained HTML (inline CSS:
...
1. Gatherer: detector + progress → local run με creds Dionisis, verify σωστά active projects + counts vs DynamoDB.
2. Gatherer: cost + perf + overprovisioning → verify νούμερα vs Console.
3. Gatherer: packer + S3 PutObject + EventBridge cron → verify `pack.json` εμφανίζεται καθημερινά.
4. IAM: gatherer role + Claude-side S3 key + bucket → verify least-privilege (deny ό,τι εκτός).
5. Routine: read pack → render HTML (`--dry-run` local) → verify HTML.
6. Routine: S3 upload + presign + Slack → verify link φτάνει στο channel.
7. `/schedule` routine daily → manual trigger → verify end-to-end.
